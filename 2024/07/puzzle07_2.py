    ...
    def calculate_left_to_right(nums, ops):
        ...
        for i in range(len(ops)):
            if ops[i] == "+":
                ...
                # print(i, nums, int(str(result) + str(nums[i + 1])))
                result += int(str(result) + str(nums[i + 1])) - result
        # print(nums, ops, result)  
        return result
...
# # Run the examples and print results
# for example in examples:
    # result = is_valid_expression(example)
    # print(f"{example} --> {'Valid' if result else 'Invalid'}")